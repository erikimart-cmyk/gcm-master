-- Records engagement with published learning content without treating reading
-- as evidence of subject mastery. The authenticated user is always derived
-- from the session, never supplied by the browser.
create function public.record_review_content_progress(
...
begin
  ...
  on conflict (user_id, review_content_id) do update
...
revoke all on function public.record_review_content_progress(uuid, boolean)
  from public, anon;

grant execute on function public.record_review_content_progress(uuid, boolean)
  to authenticated;
