-- The function returns a `question_id` output column, which is also visible as
-- a PL/pgSQL variable. Qualify the assignment table column when marking the
-- assigned question as answered to avoid an ambiguous-column error.
create or replace function public.record_assigned_question_attempt(
...
begin
  ...
  for v_retry in 1..3 loop
    ...
    begin
      ...
      update public.question_assignments as assignment
      set
